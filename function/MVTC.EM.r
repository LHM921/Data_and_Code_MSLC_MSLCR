...
MVTC.EM = function(Yc, cen, distr=c('MVN','MVT'), init.par=NULL, tol=1e-5, max.iter=1000, per=10)
{
  require(mvtnorm)
...
# old observed-data log-likelihood
  ...
  for(j in 1:num.class.cen){
    ...
    if(distr=='MVT'){
      ...
      if(pjc >= 2){
        #for(s in 1: no.ind ) cdf[ind[s]] = pmvt(lower=-Inf, upper=c(Cj%*%Yc[ind[s], ]), delta=c(mu.co[,s]), sigma=Scc.o, df=DF)
    ...
    iter = 0
    if(distr == 'MVN'){
     iter.EST = c(iter, mu, Sigma[vechS])
     old.par = c(mu, Sigma[vechS])
    }
    if(distr == 'MVT'){
     iter.EST = c(iter, mu, Sigma[vechS], nu)
     old.par = c(mu, Sigma[vechS], nu)
    }
        ...
        if(pjc == 0){ 
          ...
          if(pjc == p){
            ...
            if(distr=='MVN'){
              ...
              #EX=meanvarTMD(lower=rep(-Inf, pjc),upper=c(Cj%*%Yc[ind[s], ]),mu=mu.co[,s],Sigma=Scc.o,dist="normal")
              #yc.hat[,s] = EX$mean
              #tauy2hat[,,ind[s]] = EX$EYY   
             }
            ...
            if(distr=='MVT'){
              ...
              #tau[ind[s]] = pmvt(lower=rep(-Inf, pjc), upper=yjc, mean=mu, sigma=nu/(nu+2)*Sigma, df=round(nu+2))/pmvt(lower=rep(-Inf, pjc), upper=yjc, mean=mu, sigma=Sigma, df=round(nu))
              ...
              #EX =  meanvarTMD(lower=rep(-Inf, pjc),upper=yjc,mu=mu.co[,s],Sigma=nu/(nu+2)*Sigma,dist = "t",nu = nu+2)
              #yc.hat[,s] = EX$mean 
              #tauy2hat[,,ind[s]] = tau[ind[s]] * EX$EYY
              }
            ...
            if(distr=='MVT'){
              ...
              for(s in 1: no.ind){
               ...
               #else  cdf.rate = pmvt(lower=rep(-Inf, nrow(Scc.o)), upper=yjc, mean=c(mu.co[,s]), sigma=(DF/(DF+2))*newScc.o, df=DF+2) / pmvt(lower=rep(-Inf, pjc), upper=yjc, mean=c(mu.co[,s]), sigma=newScc.o, df=DF)
               ...
               yc.hat[,s] = EX$EY
               yc2.hat[,,s] = EX$EYY
              #EX = meanvarTMD(lower=rep(-Inf, pjc),upper=yjc,mu=mu.co[,s],Sigma=DF/(DF+2)*newScc.o,dist = "t", nu = DF+2)
              #yc.hat[,s] = EX$mean
               yjo = O %*% Yc[ind[s],]
               tauy2hat[,,ind[s]] = tau[ind[s]]*(t(O)%*% yjo %*% t(yjo) %*% O + t(O) %*% yjo %*% t(yc.hat[,s]) %*% Cj + t(Cj) %*% yc.hat[,s] %*% t(yjo) %*% O + t(Cj) %*% yc2.hat[,,s] %*% Cj)
              }
...
# M-step
      ...
      if(distr=='MVT'){
#        nu = optim(par = nu, fn = MVTC.nu.fn, method = "L-BFGS-B", lower = 2+1e-6, upper =Inf, mu=mu, Sig=Sigma, Yc=Yc, cen=cen)$par
        nu = nlminb(start = nu, objective = MVTC.nu.fn, lower = 2+1e-6, upper =Inf, mu=mu, Sig=Sigma, Yc=Yc, cen=cen)$par     
...
# new observed-data log-likelihood
  ...
  for(j in 1:num.class.cen){
    ...
    if(distr == 'MVN') iter.EST = rbind(iter.EST, c(iter, mu, Sigma[vechS]))
    if(distr == 'MVT') iter.EST = rbind(iter.EST, c(iter, mu, Sigma[vechS], nu))

    # if (iter%%per == 0){
    # if(distr=='MVN') cat("iter", iter, ":", "log-likelihood =", loglik.new, "\t diff =", diff, "\t diff.para =", diff.par, '\t mu=', mu, "\n")
    # if(distr=='MVT') cat("iter", iter, ":", "log-likelihood =", loglik.new, "\t diff =", diff, "\t diff.para =", diff.par, '\t nu=', nu, '\t mu=', mu, "\n")
    # }
   ...
   Mtau = matrix(rep(1/tau, each=p), nrow=n, ncol=p, byrow=T) # add for yhat
   yhat = tauyhat * Mtau # add yhat
   ychat = as.vector(t(yhat))[which(as.vector(t(cen))==1)] #add ychat
   IM = I.MVTC(para.est, Yc, cen=cen, distr=distr)
   cat(paste(rep("-", 50), collapse = ""), "\n")
   return(list(run.sec = run.sec, iter=iter, model.inf = model.inf, para.est = para.est, EST = EST, iter.lnL = iter.lnL, IM=IM, iter.EST = iter.EST, yhat = yhat, ychat = ychat))
}

# MVT log-likelihood function for nu
MVTC.nu.fn = function(par, mu, Sig, Yc, cen)
{
  nu = par
  n = nrow(Yc)
  p = ncol(Yc)
  ...
  for(j in 1:num.class.cen){
    ...
    if(pjc == p){          
      ...
      Soo = t(O) %*% solve(OSO) %*% O
      delta.o[ind] = colSums(cent.ind * (Soo %*% cent.ind))
      mu.co = Cj %*% (mu + Sig %*% Soo %*% cent.ind)
      Scc.o = Cj %*% (Ip - Sig %*% Soo) %*% Sig %*% t(Cj)
    }
    DF = round(nu+po[ind][1])
    if(pjc == 1){
     for(s in 1: no.ind) cdf[ind[s]] = pt(((Cj%*%Yc[ind[s], ])-mu.co[,s])/sqrt(Scc.o),df=DF)
...
I.MVTC = function(para.est, Yc, cen, distr=c('MVN','MVT'))
{
  GB = GenzBretz(maxpts = 5e4, abseps = 1e-9, releps = 0)
  n = nrow(Yc)
  p = ncol(Yc)
# parameter estimates
    ...
    if(pjc == 0){ 
      ...
      if(pjc == p){
        ...
        if(distr=='MVT'){
          ...
          yjc = c(Cj%*%Yc[ind[s], ])
          tau[ind[s]] = ptmvt(lowerx=rep(-Inf, pjc), upperx=yjc, mean=mu, sigma=nu/(nu+2)*Sigma, df=round(nu+2))/ptmvt(lowerx=rep(-Inf, pjc), upperx=yjc, mean=mu, sigma=Sigma, df=round(nu))
          tau2[ind[s]] = ((nu+p)*(nu+2))/(nu*(p+nu+2)) * ptmvt(lowerx=rep(-Inf, pjc), upperx=yjc, mean=mu, sigma=nu/(nu+4)*Sigma, df=round(nu+4))/ptmvt(lowerx=rep(-Inf, pjc), upperx=yjc, mean=mu, sigma=Sigma, df=round(nu))
          EX = TMSL.moment(mu=mu.co[,s], Sigma=nu/(nu+2)*Sigma, nu=nu+2, distr='MVT', a.low=rep(-Inf, pjc), a.upp=yjc)
          yc.hat[,s] = EX$EY
  ...
  if(distr=='MVT'){
    a1 = 1 + 2/(nu+p)
    for(j in 1: n) J1J2[,,j] = a1 * (tau2y2hat[,,j] - tau2yhat[j,]%*%t(mu) - mu%*%t(tau2yhat[j,]) + tau2[j]*mu%*%t(mu)) - (tauyhat[j,]-tau[j]*mu) %*% t(tauyhat[j,]-tau[j]*mu)  
  }
