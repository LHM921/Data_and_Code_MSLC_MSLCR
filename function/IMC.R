...
# MVN 
IM.MVN = function(para.est, EST, Y)
{
  ...
  for(j in 1: n){
    EIs[1:p, j] = Sig.inv %*% (Y[j, ] - mu)
    A2 = Y[j,] %*% t(Y[j,]) - Y[j,] %*% t(mu) - mu %*% t(Y[j,]) + mu %*% t(mu)
    for(k in 1: q1){
      SSdot = Sig.inv %*% dot.Sig[,,k]
      EIs[(p+k), j] = .5 * (tr(A2 %*% SSdot %*% Sig.inv) - tr(SSdot))
    }
  }
...
# MVNC
IM.MVNC = function(para.est, EST, Yc, cen)
{
  GB = GenzBretz(maxpts = 5e4, abseps = 1e-9, releps = 0)
  n = nrow(Yc)
  p = ncol(Yc)
  ...
  for(j in 1:num.class.cen){
    ...
    if(pjc == 0){
      ...
      if(pjc == p){
        mu.co = matrix(rep(mu, no.ind), ncol=no.ind)
        Scc.o = Sigma
        for(s in 1: no.ind){
        ...
        Soo = t(O) %*% solve(OSO) %*% O
        cent.ind = cent[, ind]
        mu.co = Cj %*% matrix(rep(mu, no.ind), ncol=no.ind) + Cj %*% Sigma %*% Soo %*% cent.ind
  ...
  EIs = matrix(NA, nrow=m, ncol=n)
  for(j in 1: n){
    EIs[1:p, j] = Sig.inv %*% (yhat[j, ] - mu)
    A2 = y2hat[,,j] - yhat[j,] %*% t(mu) - mu %*% t(yhat[j,]) + mu %*% t(mu)
    for(k in 1: q1){
      SSdot = Sig.inv %*% dot.Sig[,,k]
      EIs[(p+k), j] = .5 * (tr(A2 %*% SSdot %*% Sig.inv) - tr(SSdot))
    }
...
# MVT
IM.MVT = function(para.est, EST, Y)
{
  GB = GenzBretz(maxpts = 5e4, abseps = 1e-9, releps = 0)
  n = nrow(Y)
  p = ncol(Y)
  ...
  EIs = matrix(NA, nrow=m, ncol=n)
  for(j in 1: n){
    EIs[1:p, j] = Sig.inv %*% (tau[j]*Y[j, ]  - tau[j]*mu)
    A2 = (tau[j] * Y[j,]) %*% t(Y[j,]) - tau[j]*Y[j,] %*% t(mu) - (mu * tau[j]) %*% t(Y[j,]) + (tau[j] * mu) %*% t(mu)
    for(k in 1: q1){
      SSdot = Sig.inv %*% dot.Sig[,,k]
      EIs[(p+k), j] = .5 * (tr(A2 %*% SSdot %*% Sig.inv) - tr(SSdot))
    }
  }
  EIs[(1+p+q1), ] = .5 * (ka - tau - digamma(nu/2) - log(nu/2) -1)
...
# MVTC
IM.MVTC = function(para.est, EST, Yc, cen)
{
  GB = GenzBretz(maxpts = 5e4, abseps = 1e-9, releps = 0)
  n = nrow(Yc)
  p = ncol(Yc)
  ...
  EIs = matrix(NA, nrow=m, ncol=n)
  for(j in 1: n){
    EIs[1:p, j] = Sig.inv %*% (tauyhat[j, ] - tau[j]*mu)
    A2 = tauy2hat[,,j] - tauyhat[j,] %*% t(mu) - mu %*% t(tauyhat[j,]) + (tau[j] * mu) %*% t(mu)
    for(k in 1: q1){
      SSdot = Sig.inv %*% dot.Sig[,,k]
      EIs[(p+k), j] = .5 * (tr(A2 %*% SSdot %*% Sig.inv) - tr(SSdot))
    }
...
# MSL
IM.MSL = function(para.est, EST, Y)
{
  GB = GenzBretz(maxpts = 5e4, abseps = 1e-9, releps = 0)
  n = nrow(Y)
  p = ncol(Y)
...
# MSLC
IM.MSLC = function(para.est, EST, Yc, cen, ITER=20, per.iter=2)
{
  GB = GenzBretz(maxpts = 5e4, abseps = 1e-9, releps = 0)
  n = nrow(Yc)
  p = ncol(Yc)
